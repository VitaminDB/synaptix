//! `.syn`-бандл Qwen-Image 2.1 против исходного каталога diffusers: все
//! тензоры компонентов байт в байт, конфиги и токенайзер совпадают, модель
//! открывается из бандла.
//!
//! ```sh
//! QWEN21_DIR=…/Qwen-Image-2.1 QWEN21_SYN=…/qwen-image-2.1.syn \
...
use synaptix_image_qwen21::{QwenImage21Model, QwenImageSource};

fn pair() -> Option<(QwenImageSource, QwenImageSource)> {
    let dir = std::env::var("QWEN21_DIR").ok()?;
    let syn = std::env::var("QWEN21_SYN").ok()?;
...
fn bundle_matches_directory() {
    ...
    for f in AUX {
        let x = dir.read_opt(f).unwrap_or_else(|| panic!("в каталоге нет {f}"));
...
fn model_opens_from_bundle() {
    ...
    let m = QwenImage21Model::open(syn.path(), Device::Cpu, DType::F32, DType::F32).unwrap();
    eprintln!("{:?}", m.config());
    assert_eq!(m.config().num_layers, 32);
    assert_eq!(m.tokenizer().drop_idx(), 14);
}
