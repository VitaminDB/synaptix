...
impl DecoderModel {
        ...
        F: FnMut(usize, &Block) -> Result<(), ModelError>,
    {
        ...
        for idx in 0..n {
            ...
            // Стримящийся блок умирает здесь, но его буферы (в том числе
            // перемешанные копии весов, которые первое умножение кладёт в
            // default-пул) освобождаются `cuMemFreeAsync` В ХВОСТЕ очереди
            // compute-стрима — за всеми ядрами чанка. Пока очередь не дошла
            // до освобождения, пул считает память занятой, и следующий блок
            // берёт новые сегменты: на префилле 77k токенов default-пул так
            // разрастался до 5 ГБ «свободных, но недоступных» и ронял ход
            // OOM'ом (07.09.2026, qwen3.8-27b, 58/64 на карте). Синк перед
            // сбросом заставляет освобождение выполниться сразу, и память
            // уходит под следующий блок; стоимость — один синк на блок.
