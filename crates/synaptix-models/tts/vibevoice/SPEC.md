...
RTX 5090 Laptop, 1.5B, bf16, 20 шагов диффузии: **RTF 0.259** (10.1 с звука
за 2.6 с). f32 — 1.68 (у f32 нет fused-GEMM-пути в бэкенде).

Разбивка на токен (`examples/vv_bench`), мс:

| Стадия | было | стало |
|---|---|---|
| LM decode (×2: позитив + негатив) | 91.3 | 5.15 |
| диффузионная голова, 20 шагов | 173.5 | 10.3 |
| акустический стриминг-декод | 49.0 | 3.1 |
| семантический стриминг-энкод | 23.4 | 4.0 |

Что дало ускорение (правки общие для репозитория, см. коммит про
small-GEMM/GEMV):
...
Результат: голова 10.3 → 9.85 мс, e2e RTF 0.254 → 0.253. Мало, потому что
голова уже на 70% занята самими GEMM'ами (~7.2 мс из 10) — зазоры между
запусками были меньше, чем казалось по числу ядер. Гейт
