//! Fused SwiGLU FFN на pre-shuffled NVFP4 weights.
//!
//! `out = silu(W_gate · x) * (W_up · x)`. Один CUDA launch вместо двух
//! раздельных GEMV + elementwise mul + activation. X читается из gmem один
//! раз и переиспользуется в smem для обеих проекций.
...
static CACHE: OnceLock<Mutex<Vec<(usize, Arc<Nvfp4SwigluShufKernels>)>>> = OnceLock::new();
...
impl Nvfp4SwigluShufKernels {
    pub fn for_context(ctx: &Arc<CudaContext>) -> Result<Arc<Self>> {
        ...
        for f in [&w4, &w8] {
            ...
            .map_err(|e| {
                SynaptixError::Cuda(format!("set_attribute nvfp4_swiglu_shuf shared: {e:?}"))
            })?;
        }
...
/// Fused SwiGLU: `out = silu(W_gate · x) * (W_up · x)`. W_gate и W_up должны
/// быть pre-shuffled через `nvfp4_w_repack`. N (intermediate_size) общий,
/// K (hidden_size) общий.
#[allow(clippy::too_many_arguments)]
pub fn nvfp4_swiglu_shuf_f16(
    kernels: &Nvfp4SwigluShufKernels,
    ...
    if k % 64 != 0 {
        return Err(SynaptixError::Cuda(format!(
            "nvfp4_swiglu_shuf_f16: K={k} must be multiple of 64"
        )));
    }
    if n == 0 || n % 16 != 0 {
        return Err(SynaptixError::Cuda(format!(
            "nvfp4_swiglu_shuf_f16: N={n} must be positive and multiple of 16"
        )));
    }
    let (kfn, threads, m_tile) = if n % W8_M_TILE == 0 {
        ...
        return Err(SynaptixError::Cuda(format!(
            "nvfp4_swiglu_shuf_f16: N={n} must be multiple of 64 (W4) or 128 (W8)"
        )));
    };
    let grid = n / m_tile;
    ...
    unsafe {
        b.launch(cfg)
            .map_err(|e| SynaptixError::Cuda(format!("launch nvfp4_swiglu_shuf: {e:?}")))?;
    }
    Ok(())
}
