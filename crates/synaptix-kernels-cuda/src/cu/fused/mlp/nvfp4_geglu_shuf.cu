...
// Fused NVFP4 GEGLU FFN на shuffled W layout.
//
// Computes: out = gelu_tanh(W_gate @ x) * (W_up @ x)
// gelu_tanh(x) = 0.5 * x * (1 + tanh(sqrt(2/pi) * (x + 0.044715 * x^3)))  — pytorch_tanh / Gemma.
// W_gate, W_up — (N, K) pre-shuffled NVFP4. K общий, N общий.

#define GELU_TANH_C0 0.7978845608028654f   // sqrt(2/pi)
#define GELU_TANH_C1 0.044715f

__device__ __forceinline__ float gelu_tanh(float v) {
    float v3 = v * v * v;
    return 0.5f * v * (1.f + tanhf(GELU_TANH_C0 * (v + GELU_TANH_C1 * v3)));
}

template <unsigned int WARPS>
__device__ __forceinline__ void mma_tile_acc_geglu(
...
template <unsigned int WARPS>
__device__ __forceinline__ void geglu_shuf_impl(
    ...
    unsigned int sf_inner_dim_w)
{
    ...
    mma_tile_acc_geglu<WARPS>(packed_w_gate, scales_w_gate, smem_x, scales_x,
                              K, sf_inner_dim_w, warp, lane, m_warp_base, g_top, g_bot);
    mma_tile_acc_geglu<WARPS>(packed_w_up, scales_w_up, smem_x, scales_x,
    ...
    if ((lane & 3u) == 0u) {
        ...
        if (m_top_g < N) out[m_top_g] = __float2half(gelu_tanh(g_top) * u_top);
        if (m_bot_g < N) out[m_bot_g] = __float2half(gelu_tanh(g_bot) * u_bot);
    }
}

extern "C" __global__ void nvfp4_geglu_shuf_f16_w4(
    const unsigned char* __restrict__ packed_w_gate,
    const unsigned char* __restrict__ scales_w_gate,
    const unsigned char* __restrict__ packed_w_up,
    const unsigned char* __restrict__ scales_w_up,
    ...
    unsigned int sf_inner_dim_w)
{
    geglu_shuf_impl<4>(packed_w_gate, scales_w_gate, packed_w_up, scales_w_up,
                       packed_x, scales_x, out, N, K, sf_inner_dim_w);
}

extern "C" __global__ void nvfp4_geglu_shuf_f16_w8(
    const unsigned char* __restrict__ packed_w_gate,
    const unsigned char* __restrict__ scales_w_gate,
    const unsigned char* __restrict__ packed_w_up,
    const unsigned char* __restrict__ scales_w_up,
    ...
    unsigned int sf_inner_dim_w)
{
    geglu_shuf_impl<8>(packed_w_gate, scales_w_gate, packed_w_up, scales_w_up,
                       packed_x, scales_x, out, N, K, sf_inner_dim_w);
}
