...
extern "C" __global__ void mxfp8_quant_natural(const syn_in_t *__restrict__ in,
  ...
  const syn_in_t *xp = in + (long)row * K + (long)kblock * 32;
  float amax = 0.f;
#pragma unroll
  for (int i = 0; i < 32; i++)
    amax = fmaxf(amax, fabsf(SYN_IN_TO_F(xp[i])));
...
#pragma unroll
  for (int i = 0; i < 32; i++)
    op[i] = __nv_fp8_e4m3(fminf(fmaxf(SYN_IN_TO_F(xp[i]) / sv, -448.0f), 448.0f));
...
extern "C" __global__ void mxfp8_quant_natural_fast(const syn_in_t *__restrict__ in,
                                                    __nv_fp8_e4m3 *__restrict__ out_fp8,
                                                    unsigned char *__restrict__ out_scales,
                                                    int rows, int K) {
  const int kb_total = K / 32;
...
extern "C" __global__ void mxfp8_quant_natural_fast4(const syn_in_t *__restrict__ in,
                                                     __nv_fp8_e4m3 *__restrict__ out_fp8,
                                                     unsigned char *__restrict__ out_scales,
                                                     int rows, int K) {
  const int kb_total = K / 32;
