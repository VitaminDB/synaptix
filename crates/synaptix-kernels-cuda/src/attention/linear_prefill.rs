...
pub fn linear_attn_chunk_prefill_u8_dev(
    ...
    let mut qe = stream
        .alloc_zeros::<f32>(num_v as usize * tp * hk as usize)
        .map_err(|e| aerr("qe", e))?;
    let mut ke = stream
        .alloc_zeros::<f32>(num_v as usize * tp * hk as usize)
        .map_err(|e| aerr("ke", e))?;
    let mut vv = stream
        .alloc_zeros::<f32>(num_v as usize * tp * hv as usize)
        .map_err(|e| aerr("vv", e))?;
    let mut g_buf = stream
        .alloc_zeros::<f32>(num_v as usize * tp)
        .map_err(|e| aerr("g", e))?;
    let mut beta_buf = stream
        .alloc_zeros::<f32>(num_v as usize * tp)
    ...
    let mut state_o = stream
        .alloc_zeros::<f32>(n_state)
        .map_err(|e| aerr("ssm_state owned", e))?;
    let mut out_o = stream
        .alloc_zeros::<f32>(n_out)
        .map_err(|e| aerr("out owned", e))?;
    {
