...
use synaptix_kernels_cuda::fused::geglu::{nvfp4_geglu_shuf_f16, Nvfp4GegluShufKernels};
...
fn gelu_tanh(v: f32) -> f32 {
    const C0: f32 = 0.7978845608028654;
    const C1: f32 = 0.044715;
    let v3 = v * v * v;
    0.5 * v * (1.0 + (C0 * (v + C1 * v3)).tanh())
}
...
fn run(
    ...
    let glu = Nvfp4GegluShufKernels::for_context(ctx).expect("geglu");

    let w_g_h = det_f16(0x81, (n * k) as usize, 0.3);
    let w_u_h = det_f16(0x82, (n * k) as usize, 0.3);
    let x_h = det_f16(0x83, k as usize, 0.3);
    ...
    let ref_f32: Vec<f32> = (0..n as usize)
        .map(|i| gelu_tanh(g_h[i].to_f32()) * u_h[i].to_f32())
        .collect();
    ...
    eprintln!("[geglu {name} N={n} K={k}] cos={cos:.6} max_abs={max_abs:.4} (rel={rel:.4})");
...
#[test]
fn geglu_w8_gemma_ish() {
    let Some((ctx, stream)) = setup() else { return };
    run(&ctx, &stream, 6144, 2048, "gemma", 0.99);
}

#[test]
fn geglu_w8_large() {
    let Some((ctx, stream)) = setup() else { return };
    run(&ctx, &stream, 27648, 5120, "large", 0.99);
}
