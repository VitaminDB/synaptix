...
/// DAC — Descript Audio Codec (Kumar et al. 2024).
///
/// Минимальный composite: `encoder_proj → RVQ → decoder_proj`. Полный DAC
/// использует snake-activation conv-stack (откладывается до Phase O), здесь только
/// quantization-pipeline — `forward` корректно проходит через RVQ и
/// восстанавливает форму. Параметры по умолчанию: 9 codebook'ов × 1024
/// entries × 8-dim — это уровни DAC-tiny; реальный DAC-44kHz использует
/// dim=8/12 на 9 кодбуков.
pub struct Dac {
