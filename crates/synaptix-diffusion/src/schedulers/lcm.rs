...
impl Scheduler for LcmScheduler {
    fn set_timesteps(&mut self, n_steps: usize) -> Result<()> {
        ...
        let k = (n_train / self.cfg.original_inference_steps).max(1);
        let c_idx = (self.cfg.original_inference_steps.min(n_train)) / n_steps.max(1);
        let mut indices: Vec<usize> = (0..n_steps)
            .map(|i| ((n_steps - 1 - i) * c_idx * k).min(n_train - 1))
            .collect();
        indices.sort_by(|a, b| b.cmp(a));
        indices.dedup();
        self.timestep_indices = indices;
        self.timesteps = self.timestep_indices.iter()
            .map(|&i| i as f32 * self.cfg.timestep_scaling)
            .collect();
    ...
    fn step(&mut self, model_output: &Tensor, step_idx: usize, sample: &Tensor) -> Result<SchedulerOutput> {
        ...
        let t_prev = if step_idx + 1 < self.timestep_indices.len() { self.timestep_indices[step_idx + 1] } else { 0 };
        let alpha_bar = self.alphas_cum[t_idx];
        let alpha_bar_prev = self.alphas_cum[t_prev];
        let sigma = ((1.0 - alpha_bar) / alpha_bar.max(1e-12)).sqrt();
        ...
        let prev_sample = x0.affine(alpha_bar_prev.max(1e-12).sqrt(), 0.0)?
            .add(&noise.affine((1.0 - alpha_bar_prev).max(0.0).sqrt(), 0.0)?)?;
