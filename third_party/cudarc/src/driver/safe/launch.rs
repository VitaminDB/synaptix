...
unsafe impl<'a, 'b: 'a, T> PushKernelArg<&'b CudaSlice<T>> for LaunchArgs<'a> {
    ...
    fn arg(&mut self, arg: &'b CudaSlice<T>) -> &mut Self {
        // synaptix fork: home-stream == launch-stream → wait/record избыточны
        // (stream-ordering даёт ту же гарантию); кросс-стрим связи сохраняются.
        if self.stream.context().is_managing_stream_synchronization()
            && arg.stream.cu_stream() != self.stream.cu_stream()
        {
...
impl LaunchArgs<'_> {
    ...
    pub unsafe fn launch(
        ...
        // Phase D Step 10 (synaptix fork): под stream capture обычные
        // stream.wait(event) и event.record(stream) пересекают capture
        // boundary → CUDA_ERROR_STREAM_CAPTURE_ISOLATION. CUDA docs: события
        // записанные на uncaptured work не могут быть waited из captured
        // graph'а; для event.record() во время capture'а нужен флаг
        // CU_EVENT_RECORD_EXTERNAL (запись на host stream, не в граф).
        ...
        let end_event = self
            ...
            .transpose()?;
        if capture_active {
            // Phase E.6: skip event recording полностью (даже EXTERNAL)
            // — EVENT_RECORD nodes в captured graph → ILLEGAL_ADDRESS на
            // replay'е. Sync-семантика внутри stream'а сохраняется (sequential).
        } else {
    ...
    pub unsafe fn launch_cooperative(
        ...
        self.stream.ctx.bind_to_thread()?;
        // См. комментарий в `launch()` про capture-aware events.
        let capture_active = matches!(
            self.stream.capture_status(),
            Ok(sys::CUstreamCaptureStatus::CU_STREAM_CAPTURE_STATUS_ACTIVE)
        );
