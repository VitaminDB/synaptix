        ...
        match self {
            SyncOnDrop::Record(target) => {
                if let Some((event, stream)) = std::mem::take(target) {
                    // Phase D Step 10 (synaptix fork): под stream capture обычный
                    // cuEventRecord пересекает capture boundary →
                    // CUDA_ERROR_STREAM_CAPTURE_ISOLATION.
                    //
                    // Phase E.6: даже `cuEventRecordWithFlags(EXTERNAL)`
                    // оставляет EVENT_RECORD nodes в captured graph (~2× от
                    // числа KERNEL'ов — в нашем decode step ~3460 events на
                    // 1843 kernels). Эти node'ы на replay'е дают
                    // CUDA_ERROR_ILLEGAL_ADDRESS — событие пытается
                    // отрабатываться на каждом replay'е, но event handle уже
                    // мог быть переиспользован/перезаписан между replay'ями.
                    //
                    // Решение: под capture **полностью skip** event recording.
                    // Sync-семантика внутри stream'а сохраняется автоматически
                    // (kernel'ы в одном stream'е sequential by definition →
                    // event для read-after-write не нужен). После capture
                    // (replay-time) события для post-capture sync уже не
                    // актуальны — graph capture снимает immutable копию
                    // sequence operations.
                    match stream.capture_status() {
                        Ok(sys::CUstreamCaptureStatus::CU_STREAM_CAPTURE_STATUS_ACTIVE) => {
                            // skip — никаких EVENT_RECORD nodes в graph
                            let _ = event;
                        }
                        _ => {
            ...
            SyncOnDrop::Sync(target) => {
                if let Some(stream) = std::mem::take(target) {
                    // Под capture stream.synchronize() запрещён CUDA docs
                    // (sync API не работает в captured stream). Skip — read'у
                    // не нужен sync после kernel'а внутри того же graph'а.
                    match stream.capture_status() {
                        Ok(sys::CUstreamCaptureStatus::CU_STREAM_CAPTURE_STATUS_ACTIVE) => {}
                        _ => {
...
impl<T> DevicePtr<T> for CudaSlice<T> {
    fn device_ptr<'a>(&'a self, stream: &'a CudaStream) -> (sys::CUdeviceptr, SyncOnDrop<'a>) {
        // synaptix fork: home-stream == целевой стрим → wait/record избыточны
        // (stream-ordering); кросс-стрим связи сохраняются.
        if self.stream.cu_stream() == stream.cu_stream() {
            return (self.cu_device_ptr, SyncOnDrop::Record(None));
        }
...
impl<T> DevicePtr<T> for CudaView<'_, T> {
    fn device_ptr<'a>(&'a self, stream: &'a CudaStream) -> (sys::CUdeviceptr, SyncOnDrop<'a>) {
        if self.stream.cu_stream() == stream.cu_stream() {
            return (self.ptr, SyncOnDrop::Record(None));
        }
        if self.stream.context().is_managing_stream_synchronization() {
            if let Some(write) = self.write.as_ref() {
                stream.ctx.record_err(stream.wait(write));
